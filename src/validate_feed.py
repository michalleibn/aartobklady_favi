...
assert len(items) == 4, f"Očekávány 4 produkty, nalezeno {len(items)}"
ids = []
for item in items:
    for tag in REQUIRED:
        assert item.find(tag) is not None and (item.findtext(tag) or "").strip(), f"Chybí {tag}"
    ids.append(item.findtext("ITEM_ID").strip())
assert set(ids) == {"71", "205", "689", "206"}, f"Neočekávané ITEM_ID: {ids}"
assert len(ids) == len(set(ids)), "Duplicitní ITEM_ID"
print(f"OK: {path} obsahuje produkty 71, 205, 689 a 206 a všechny povinné elementy.")
